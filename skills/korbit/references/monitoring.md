...
`korbit monitor` is the one **streaming** command — a resilient WebSocket client (auto-reconnect,
...
data events *and* notices — exactly like `korbit monitor --json | jq`. It **implies `--json`**, and
...
korbit monitor --symbols btc_krw --ticker --max-events 1 --stream-log-level warn \
...
```sh
korbit monitor --symbols btc_krw,eth_krw --ticker --duration 1h --stream-log-level warn \
...
```sh
korbit monitor --symbols btc_krw --my-orders --my-trades --my-assets --json
```
...
```sh
korbit monitor --symbols btc_krw --trades --orderbook --duration 2m --json > /tmp/btc_stream.ndjson
...
```sh
korbit monitor --symbols btc_krw --my-orders --jq 'select(.type=="notice" and (.code=="DATA_GAP" or .code=="DISCONNECTED"))'
```
...
```sh
korbit monitor --symbols btc_krw,eth_krw,xrp_krw --ticker --duration 10m --json
```
...
```sh
korbit monitor --symbols btc_krw --candles 1 --candle-history 100 --stream-log-level warn \
