...
var Registry = []Command{
	{
		...
		ExperimentalNotes: []string{
			...
			"The api.* API in --init/--on mirrors the CLI commands (api.ticker, api.order.place/get/cancel/open/history, api.balance, api.fills, api.deposit.*, api.withdraw.*, api.krw.*, ...): every method takes one options object keyed on the wire parameter names from `{prog} commands` (the obvious single positional like a symbol may be passed directly) and returns a Promise of the response data. Price/qty/amount values MUST be strings. Failures reject with err.code/err.httpStatus/err.description/err.body. The global is also bound as `korbit` — the same object under a second name, so a script written against either works.",
