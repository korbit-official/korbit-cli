...
var GlobalFlags = []GlobalFlag{
	{Flag: "key", TakesValue: true, Desc: "named key to sign with (default: the key set via `{prog} key use`; also: DIGITALX_CLI_KEY). Or supply a key inline via DIGITALX_CLI_API_KEY_ID + DIGITALX_CLI_API_KEY_SECRET + DIGITALX_CLI_API_KEY_TYPE — mutually exclusive with --key/DIGITALX_CLI_KEY"},
