...
const (
	...
	// exclusive with a stored-key selection (--key / KORBIT_CLI_KEY).
	EnvAPIKeyID     = "KORBIT_CLI_API_KEY_ID"
	EnvAPIKeySecret = "KORBIT_CLI_API_KEY_SECRET"
	EnvAPIKeyType   = "KORBIT_CLI_API_KEY_TYPE"
)
...
// (KORBIT_CLI_API_KEY_*) is set — i.e. the invocation is in inline-credential
...
func InlineCredsPresent(getenv func(string) string) bool {
	return getenv(EnvAPIKeyID) != "" || getenv(EnvAPIKeySecret) != "" || getenv(EnvAPIKeyType) != ""
}

// Select determines the credential plan from the --key flag value and the
// environment, enforcing that inline env material (KORBIT_CLI_API_KEY_*) and a
// stored-key selection (--key / KORBIT_CLI_KEY) are MUTUALLY EXCLUSIVE. It reads
...
func Select(flagKey string, getenv func(string) string) (Selection, error) {
	...
	named := flagKey != "" || getenv(EnvKeyName) != ""
	if inline && named {
...
// straight from the KORBIT_CLI_API_KEY_* vars without any keystore access.
...
func resolveInline(getenv func(string) string) (Resolved, error) {
	id := strings.TrimSpace(getenv(EnvAPIKeyID))
	secret := getenv(EnvAPIKeySecret)
	keyType := strings.TrimSpace(getenv(EnvAPIKeyType))
	switch {
	case id == "":
