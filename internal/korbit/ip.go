...
// ipPath is the public endpoint that echoes the caller's source IP as Korbit
...
// family is what lets the CLI report both the IPv4 and the IPv6 address Korbit
...
// User-Agent; an empty string falls back to the bare "korbit-cli/<version>".
...
// of its own. An empty userAgent falls back to "korbit-cli/<version>".
func ProbeIP(ctx context.Context, doer Doer, baseURL, userAgent string) (string, error) {
	if userAgent == "" {
		userAgent = "korbit-cli/" + version.Version
	}
