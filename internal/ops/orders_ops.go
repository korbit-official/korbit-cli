...
func init() {
	...
	register(placeOp{meta: OpMeta{
		...
		Notes: []string{
			...
			"Notional bounds are per market, in the pair's quote currency: read this pair's own minOrderValue/maxOrderValue (and its quoteCurrency) from `{prog} pairs` and check price*qty (or amt) against them. A bound the pair omits is a figure it does not publish, not a guarantee of none: skip that check and let the server decide, and never apply another market's figure.",
			"Preview with --dry-run: it prints the unsigned request AND runs a customer-protection check against live public market data (orderbook + tick size + the pair's order value bounds, fetched from the same base URL the order uses — no credentials touched). The output's `warnings` flag risky orders: high market-order slippage / insufficient liquidity, a limit price far from market (fat-finger), a post-only that would be rejected for crossing, a fill-or-kill that would be killed, a tick-misaligned price, an out-of-bounds notional, or an order that sweeps past the visible book depth (its fill/slippage estimate is only a lower bound). Every book shape is analyzed, including an empty or one-sided one: an order that cannot rest there is flagged as having nothing to fill against, and a check needing a reference price the book cannot supply is flagged as not run — so an empty `warnings` alone is not proof the price is sound. A `bestBid`/`bestAsk`/`mid` the book cannot supply is reported empty, never zero. The checks are omitted entirely (`checksSkipped`) only when the market data cannot be fetched. Advisory only — never blocks the order.",
		},
